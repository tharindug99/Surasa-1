...
const AdminDetail = (props) => {
  // Renamed from Landing to AdminDetail
  const { setAdmins, admins } = props; // Add admins to props
  const [loading, withLoading] = useLoading();

  const getAdmins = async () => {
    // New function for fetching admins
    try {
  ...
  useEffect(() => {
    if (admins?.length < 1) {
      // Fetch admins if not already fetched
      getAdmins();
    }
  ...
  return (
    <>{loading ? "Loading Admins" : <h4>Check console for admins data</h4>}</>
  );
};

const mapStateToProps = ({ admin }) => {
  // Add admin to mapStateToProps
